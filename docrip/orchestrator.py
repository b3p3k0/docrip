...
from .util import ensure_dir, write_json, utc_datestr, base36_digest5, host_identifier, clamp
...
def auto_workers(explicit: int) -> int:
    if explicit and explicit>0: return explicit
...
def process_one(cfg: Config, v: Volume, token: str, date_str: str, comp_threads_job: int, dry=False) -> VolumeResult:
    ...
    try:
        if not mount_ro(v, mp, dry=dry):
            status = "mount_failed"
            return VolumeResult(v.path, v.fstype, v.size_bytes, name, str(mp), status, round(time.time()-started,2))
        ok = make_chunks(cfg, mp, work_root, compressor_threads=comp_threads_job, dry=dry)
        if not ok:
            status = "chunk_failed"
            return VolumeResult(v.path, v.fstype, v.size_bytes, name, str(mp), status, round(time.time()-started,2))
        ...
        return VolumeResult(v.path, v.fstype, v.size_bytes, name, str(mp), status, round(time.time()-started,2), error=error)
    finally:
        umount(mp, dry=dry)

def run_plan(cfg: Config, only: set[str] | None, list_only: bool, workers_override: int | None, dry: bool):
    prepend_bin_to_path()
    ...
    workers = auto_workers(workers_override if workers_override is not None else cfg.workers)
    comp_thr = comp_threads_for(workers)
    print(f"[info] workers={workers} comp_threads/job≈{comp_thr} date={date_str} token={token}")

    results: List[VolumeResult] = []
    start=time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
        futs = [ex.submit(process_one, cfg, v, token, date_str, comp_thr, dry) for v in to_process]
    ...
    if failed:
        print(f"[warn] {len(failed)} volume(s) failed or partial. See JSON logs in {cfg.run_summary_dir}.", file=sys.stderr)
        return 1
