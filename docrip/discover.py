...
def lsblk_json() -> Dict[str, Any]:
    rc,out = run(["lsblk","-b","-J","-o","NAME,KNAME,PATH,TYPE,SIZE,FSTYPE,FSVER,LABEL,UUID,MOUNTPOINT,RM,RO,MODEL,TRAN"], capture=True)
    if rc!=0: raise RuntimeError("lsblk failed")
...
def is_encrypted(dev: str, fstype: str) -> bool:
    ...
    if fstype == "crypto_LUKS": return True
    info = blkid_export(dev)
    t = info.get("TYPE","").lower()
    label = info.get("LABEL","").lower()
    if "crypto_luks" in t: return True
    if "bitlocker" in t or "bitlocker" in label or "fve" in label: return True
    if t == "apfs" and "encrypted" in info.get("APFS_FEATURES","").lower(): return True
    if "veracrypt" in label or "truecrypt" in label: return True
    return False
...
def collect_volumes(cfg: Config) -> List[Volume]:
    ...
    def walk(node: Dict[str,Any]):
        ...
        if t in consider or (t=="disk" and fstype):
            ...
            m = re.search(r"(\d+)$", kname or ""); partno = int(m.group(1)) if m else 0
            vols.append(Volume(path,kname,fstype,size,t,uuid,enc,diskno,partno,model))
...
def print_plan(vols: List[Volume]) -> None:
    ...
    print(f"{'DEVICE':<20} {'FS':<8} {'SIZE(GB)':>9} {'DISK':>4} {'PART':>4} {'STATUS':<20}")
    for v in sorted(vols, key=lambda x:(x.diskno, x.partno, x.path)):
        gb = v.size_bytes/(1024**3)
        status = v.skip_reason or "process"
        print(f"{v.path:<20} {v.fstype or '-':<8} {gb:>9.1f} {v.diskno:>4} {v.partno:>4} {status:<20}")
