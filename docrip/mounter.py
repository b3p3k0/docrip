...
def mount_ro(v: Volume, mp: Path, dry: bool=False) -> bool:
    ...
    if fs in ("ext2","ext3","ext4"):
        cmd = ["mount","-t","ext4","-o","ro,noload,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="xfs":
        cmd = ["mount","-t","xfs","-o","ro,norecovery,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="btrfs":
        cmd = ["mount","-t","btrfs","-o","ro,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="ntfs":
        cmd = ["ntfs-3g","-o","ro,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="vfat":
        cmd = ["mount","-t","vfat","-o","ro,uid=0,gid=0,umask=022,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="exfat":
        cmd = ["mount","-t","exfat","-o","ro,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="hfs":
        cmd = ["mount","-t","hfs","-o","ro,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="hfsplus":
        cmd = ["mount","-t","hfsplus","-o","ro,force,nodev,nosuid,noexec",v.path,str(mp)]
    elif fs=="apfs":
