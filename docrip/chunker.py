...
def make_chunks(cfg: Config, mp: Path, outdir: Path, compressor_threads: int, dry: bool=False) -> bool:
    ...
    if cfg.chunk_size_mb and cfg.chunk_size_mb > 0:
        ...
        run(f"for p in {str(outdir)!s}/*.part*; do sha256sum \"$p\" > \"$p.sha256\"; done", dry=dry)
        run(f"ls -1 {str(outdir)!s}/*.part* | sort | sed 's#^.*/##' > {str(parts_list)!s}", dry=dry)
